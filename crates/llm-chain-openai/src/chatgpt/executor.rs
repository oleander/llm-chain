...
impl traits::Executor for Executor {
  ...
  fn tokens_used(&self, opts: &Options, prompt: &Prompt) -> Result<TokenCount, PromptTokensError> {
    ...
    let tokens_used = num_tokens_from_messages(model, &messages).map_err(|_| PromptTokensError::NotAvailable)?;

    let s = self.max_tokens_allowed(opts);

    Ok(TokenCount::new(s, tokens_used as i32))
  }
...
fn num_tokens_from_messages(
  ...
  for message in messages {
    let (role, content, name) = match message {
      ChatCompletionRequestMessage::System(x) => (x.role.to_string(), x.content.to_owned().unwrap_or_default(), None),
      ...
      ChatCompletionRequestMessage::Assistant(x) => {
        (x.role.to_string(), x.content.to_owned().unwrap_or_default(), None)
      }
      ChatCompletionRequestMessage::Tool(x) => (x.role.to_string(), x.content.to_owned().unwrap_or_default(), None),
      ChatCompletionRequestMessage::Function(x) => (x.role.to_string(), x.content.to_owned().unwrap_or_default(), None),
    };
...
impl OpenAITokenizer {
  ...
  pub fn for_model_name<S: Into<String>>(model_name: S) -> Self {
    Self {
      model: ModelOpt::new(model_name.into()),
    }
  }
}
